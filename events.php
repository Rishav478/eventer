			...
			<div class="container">
				...
				<div class="row">
					...
					<div id="store" class="col-md-12">
						...
						<div class="row">
						<?php
							$sql = "SELECT * from event where event_price = 0 and event_status = 'Active' order by event_id desc";
			...
			<div class="container">
				...
				<div class="row">
					...
					<div id="store" class="col-md-12">
						...
						<div class="row">
						<?php
							$sql = "SELECT * from event where event_status = 'Active' order by event_id desc";
							$result = mysqli_query($con,$sql);
							while ($row = mysqli_fetch_array($result)) {
								$rDate = strtotime($row['event_regEndDate']);
  								$cDate = strtotime(date('Y-m-d'));
						...
						<?php
							...
							while ($row = mysqli_fetch_array($result)) {
								$rDate = strtotime($row['event_regEndDate']);
  								...
  								if ($cDate <= $rDate) {
						?>
							<!-- product --><a href="event-details.php?name=<?= $row['paralLink'] ?>&value=<?= password_hash($paralLink, PASSWORD_DEFAULT) ?><?= password_hash($paralLink, PASSWORD_DEFAULT) ?><?= password_hash($paralLink, PASSWORD_DEFAULT) ?><?= password_hash($paralLink, PASSWORD_DEFAULT) ?>">
							<div class="col-md-4 col-xs-6">
								<div class="product">
									<div class="product-img">
										<img src="seller/assets/uploads/event-banner/<?= $row['event_banner'] ?>" height="280px" alt="">
